...
//审题要清楚，一棵树，自己和自己比较
bool IsEqualBinaryTree(BinaryTreeNode* pRoot1, BinaryTreeNode* pRoot2)
{
    ...
    else if (pRoot1 != nullptr && pRoot2 != nullptr)
    {
        ...
        {
            return  IsEqualBinaryTree(pRoot1->m_pLeft, pRoot2->m_pRight) && IsEqualBinaryTree(pRoot1->m_pRight, pRoot2->m_pLeft);
        }
        else 
        {
            return false;
...
void Test(char* testName, BinaryTreeNode* pRoot, bool expected)
{
    ...
    if (IsEqualBinaryTree(pRoot, pRoot) == expected)
    {
        printf("Passed. \r\n");
    }
    else
